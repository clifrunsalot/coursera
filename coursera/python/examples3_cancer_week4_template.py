...
def select_columns(my_table, col_indices):
    ...
    out_table = []
    for row in my_table:
        tmp_row = [row[idx] for idx in col_indices]
        out_table.append(tmp_row)
    return out_table
...
def test_part2_code():
    ...
    col_1 = 0
    col_2 = 2
    print("Subset of original with columns: " + str(col_1) + "," +str(col_2))
    print_table(select_columns(test_table, [col_1, col_2]))
    print()
    # Simple test for column trimmng function
    col_1 = 1
    col_2 = 3
    ...
    # Simple test for column sorting function descending
    sort_col = 3
    print("Sort by col " + str(sort_col) + " descending")
    sort_by_column(test_table, sort_col)
    print_table(test_table)
    print()
    ...
    print("Tables are equal: " + str(trimmed_risk_table == trimmed_risk_solution))
